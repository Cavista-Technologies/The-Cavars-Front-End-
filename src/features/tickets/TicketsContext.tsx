...
const PAGE_SIZE = 20;
const STATUS_BY_NUMBER: Record<number, TicketStatus> = {
  0: "open",
  1: "claimed",
  2: "resolved",
};
  ...
  const normalize = React.useCallback(
    (remote: RemoteTicket[]): Ticket[] => {
      const existingById = new Map(
        stateRef.current.tickets.map((ticket) => [ticket.id, ticket]),
      );
      ...
      return remote.map((ticket) => {
        ...
        const assignee = ticket.assignedTo
          ? users.find(
              (user) =>
                ...
                user.emailAddress === ticket.assignedToEmail,
            )
          : undefined;
        ...
        return {
          ...
          raisedByEmail:
            owner?.emailAddress ??
            (isCurrentUserOwner ? authUser?.email : null) ??
            "Unknown employee",
          raisedByName:
            ticket.ownerName ||
            owner?.fullName ||
            (isCurrentUserOwner ? authUser?.name : null) ||
            "Unknown employee",
